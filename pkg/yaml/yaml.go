...
func ParseObjects(manifest string) ([]*unstructured.Unstructured, error) {
	...
	for {
		...
		if err == io.EOF {
			...
			return nil, errors.Wrap(err, "Could not read yaml manifest")
		}

		jsondata, err := yaml.ToJSON(objManifest)
		if err != nil {
			return nil, errors.Wrap(err, "Could not parse yaml manifest to json")
		}
		...
		if err != nil {
			return nil, errors.Wrap(err, "Could not decode json manifest to k8s object")
		}
		ret = append(ret, obj)
	}

...
interface IAppControlsProps {
  app: IRelease;
  deleteApp: (keepHistory: boolean) => Promise<boolean>;
...
class AppControls extends React.Component<IAppControlsProps, IAppControlsState> {
  ...
  public handleDeleteClick = async () => {
    ...
    const deleted = await this.props.deleteApp(!alreadyDeleted && this.state.keepHistory);
  ...
  private togglePurge = () => {
    this.setState({ keepHistory: !this.state.keepHistory });
  };
}

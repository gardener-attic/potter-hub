...
> {
  ...
  public render() {
    const { selected, chartID, chartVersion, goBack, namespace } = this.props;
      ...
      <div>
          ...
          >
            {versions.map(v => (
              <option key={v.id} value={v.attributes.version}>
                {`Chart: ${v.attributes.version} | App: ${v.attributes.app_version || "-"} `} 
        ...
        <div className="margin-t-big">
          <button
            ...
            type="submit"
            disabled={!namespace || namespace === definedNamespaces.all}
          >
            Submit
          </button>
          ...
          {(!namespace || namespace === definedNamespaces.all) && (
            <div className="container-tiny" style={{ display: "flex", marginTop: "10px" }}>
              <div className="alert alert-warning AlertBox" role="alert">
                <div className="AlertBoxIconContainer">
                  <AlertOctagon />
                </div>
                Please select a namespace using the namespace selector from the top menu.
              </div>
            </div>
          )}
        </div>

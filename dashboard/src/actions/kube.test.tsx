...
describe("getResource", () => {
  it("fetches a resource", async () => {
    const expectedActions = [
      {
        ...
        payload: "api/ui-backend/reverse-proxy/v1/api/v1/namespaces/default/services/foo",
      },
      {
        type: getType(actions.kube.receiveResource),
        payload: {
          key: "api/ui-backend/reverse-proxy/v1/api/v1/namespaces/default/services/foo",
          resource: [],
        },
      },
    ];
    ...
    expect(getResourceMock).toHaveBeenCalledWith("v1", "services", false, "default", "foo");
  });
  ...
  it("does not fetch a resource that is already being fetched", async () => {
    store = mockStore({
      kube: {
        items: {
          "api/ui-backend/reverse-proxy/v1/api/v1/namespaces/default/services/foo": {
            isFetching: true,
          },
        },
      },

...
describe("App", () => {
    ...
    [
      {
        ...
        result: `${HELM_URL}/releases`,
      },
      {
        description: "returns namespaced URLs",
        namespace: "default",
        result: `${HELM_URL}/namespaces/default/releases`,
      },
      {
        description: "returns a single release URL",
        namespace: "default",
        resourceName: "foo",
        result: `${HELM_URL}/namespaces/default/releases/foo`,
      },
      {
        description: "returns a URL with a query",
        namespace: "default",
        query: "statuses=foo",
        result: `${HELM_URL}/namespaces/default/releases?statuses=foo`,
      },
    ].forEach(t => {
      it(t.description, () => {
    ...
    [
      {
        ...
        expectedURL: `${HELM_URL}/releases`,
      },
      {
        description: "should request the releases of a namespace",
        expectedURL: `${HELM_URL}/namespaces/default/releases`,
        namespace: "default",
      },
      {
        all: true,
        description: "should request the releases of a namespace with any status",
        expectedURL: `${HELM_URL}/namespaces/default/releases?statuses=all`,
        namespace: "default",
      },
    ].forEach(t => {
  ...
  describe("create", () => {
    const expectedURL = `${HELM_URL}/namespaces/default/releases`;
  ...
  describe("upgrade", () => {
    const expectedURL = `${HELM_URL}/namespaces/default/releases/absent-ant`;
    const testChartVersion: IChartVersion = {
      attributes: {
        version: "1.2.3",
      },
    ...
    [
      {
        ...
        expectedURL: `${HELM_URL}/namespaces/default/releases/foo?keepHistory=true`,
        keepHistory: true,
      },
      {
        description: "should delete and purge an app in a namespace",
        expectedURL: `${HELM_URL}/namespaces/default/releases/foo`,
        keepHistory: false,
      },
    ].forEach(t => {
      it(t.description, async () => {
        moxios.stubRequest(/.*/, { response: "ok", status: 200 });
        expect(await App.delete("foo", "default", t.keepHistory)).toBe("ok");
        expect(moxios.requests.mostRecent().url).toBe(t.expectedURL);
      });
    });
  ...
  describe("rollback", () => {
    it("should rollback an application", async () => {
      ...
      expect(axiosWithAuth.put).toBeCalledWith(
        "api/ui-backend/helm/v1/namespaces/default/releases/foo",
        {},
